##================================================================================##
##			Function for Calculating Co-cluster Means, Ranking Them  	    ##
##			and Their Corresponding Gene and Compound Clusters,	    	    ##
##			Extraction of Biomarker Co-clusters using using Individual 	    ##
##			Control Chart								    ##
##			By 										    ##
##			Dr. Mohammad Nazmol hasan 					    	    ##
##			Professor, Dept. of Statistics 		     		          ##
##			Bangabandhu Sheikh Mujibur Rahman Agricultural University       ##
##			Gazipu-1706, Bangladesh 						    ##
##================================================================================##

BioMGC <- function(dataFExpr, rk, ck)
{

dataExpr <- 100*(1/(1+exp(-dataFExpr)))  ## Data Transformation using logistic function for RHC
...
HCclsMatG <- as.matrix(cutree(HCGene, rk)) ## rk row clusters estimated observing dendrogram
HCclsMatC <- as.matrix(cutree(HCComp, ck)) ## ck column clusters estimated observing dendrogram
##----------------------------------------------------------------------------------------------

## Loop to get list of row and column clusters produced by HC  
HCclsG <- list()
HCclsC <- list()
for (p in 1:rk)
{
...
## numbers based on the proposed RHC  
NGcls <- NULL
NCcls <- NULL
GC_CoMean <- NULL 
...
## Re-arranged data matrix based on RHC recovered clusters
...
#### Calling results 
return(list(NGC_cls_MeanMat = NGC_cls_MeanMat, CoClsDtMat = CoClsDtMat,Co_Gcls = Co_Gcls, 
	Co_Ccls = Co_Ccls, colorsC = colorsC, colorsG = colorsG, CentralLine = CentralLine, 
	UpContLimit = UpContLimit, LowrContLimit = LowrContLimit, color = color, pchmark = pchmark )) 
}
